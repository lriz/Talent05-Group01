...
class sps_generator(object):
    ...
    def calc_m_broken_basis(self, shell_configurations_list):
        sps_index = 1
        for p_level_index in self.input_dict["shell-configuration P-levels"].keys():
            level_name = self.input_dict["shell-configuration P-levels"][p_level_index]["name"]
            N_number = [shell['N'] for shell in  shell_configurations_list  if shell['name'] == level_name][-1] #TODO: check that not empty.
            j_total = self.input_dict["shell-configuration P-levels"][p_level_index]["2J-total"]
            for m_j in range(-j_total, j_total+1, 2):
                self.all_sps_list.append(single_particle_state(p_level_index,
                                                             N_number,
                                                             self.input_dict["shell-configuration P-levels"][p_level_index]["angular momentum"],
                                                             j_total,
                                                             m_j,
                                                             sps_index))
                sps_index += 1
    ...
    def calc_m_scheme_basis(self, m_broken_basis):
        for sps in m_broken_basis:
            ...
            for p in self.input_dict["shell-configuration P-levels"]:
                sps_p_list = []
                for i in sps:
                    if i.get_p() == p:
                        sps_p_list.append(i)
            ...
            for p, value in sps_temp_dict.iteritems():
                if sum([v.get_m_j() for v in value]) == self.input_dict["shell-configuration P-levels"][p]["M-total"]:
                    continue
                else:
                    m_bool = False
            if m_bool:
    ...
    def print_m_scheme_basis(self):
        print
        m_total_list = [(self.input_dict["shell-configuration P-levels"][p]["M-total"], self.input_dict["shell-configuration P-levels"][p]["name"]) for p in self.input_dict["shell-configuration P-levels"]]
        m_total_list = []
        for p in self.input_dict["shell-configuration P-levels"]:
            m_total_list.append(self.input_dict["shell-configuration P-levels"][p]["M-total"])
            m_total_list.append("".join(("(",self.input_dict["shell-configuration P-levels"][p]["name"],")")))
