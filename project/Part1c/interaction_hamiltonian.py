...
class OneBodyInteraction(object):
    def __init__(self,sp_basis,m_scheme_basis,potential):
        self.sp_basis = sp_basis
        self.m_scheme_basis = m_scheme_basis
        self.potential = potential
    ...
    def _compute_connections(self,state,ind_i):
        ...
        :return:
        """
        # annihilate one particles at place i
        for i in range(0,len(state)):
            ...
            # create two particles with sp states a and b
            for a in range(1,len(self.sp_basis)+1):  #TODO: using the sp basis counting scheme? Particle #1 (index 1), particle #2 (index 2)...
                if self.sp_basis[a-1] in state_a:  #TODO: needed? won't it continue anyway?
                    continue
                # Inserts the new particles, and remember the phase
                # important to clone state_prime first
                state_b = None  # needed? Yes otherwise state_b is local within the if statement in the for loop
                phase_b = None  # needed?
                for k in range(0,len(state_a)):
                    if (a<state_a[k].get_index()):
                        state_b = np.insert(state_a,k,self.sp_basis[a-1])
                ...
                ind_j = 0
                if state_b in self.m_scheme_basis:
                    ind_j = find_index(self.m_scheme_basis.tolist(),state_b)
                    if ind_j == None:
                        continue
                else:
                    continue
                ...
                self.matrix[ind_i,ind_j]+=self.potential.get_matrix_element(self.sp_basis[a-1],c)*(((phase_a+phase_b)%2)*2-1)
                # TODO: search for the corresponding state in m_schemebasis
                # TODO: find matrix element multiply with the phase
